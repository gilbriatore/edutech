...
export const Aluno = () => {
  ...
  const totalItems = useAppSelector(state => state.aluno.totalItems);
  ...
  return (
    <div>
      <h2 id="aluno-heading" data-cy="AlunoHeading">
        <Translate contentKey="edutechApp.aluno.home.title">Alunos</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="edutechApp.aluno.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/aluno/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="edutechApp.aluno.home.createLabel">Create new Aluno</Translate>
          </Link>
        </div>
      </h2>
        ...
        {alunoList && alunoList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="edutechApp.aluno.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('nome')}>
                  <Translate contentKey="edutechApp.aluno.nome">Nome</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('nome')} />
                </th>
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/aluno/${aluno.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/aluno/${aluno.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="edutechApp.aluno.home.notFound">No Alunos found</Translate>
            </div>
          )
        )}
      </div>
